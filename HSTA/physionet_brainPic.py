...
for num in range(7, 8):
    ...
    # print(test_features_partition5.shape)  # (8100, 9, 5, 4)
    ...
    # dataset = TensorDataset(features_partition3, labels)
    ...
    # test_dataset = TensorDataset(test_features_partition3, test_labels)
    ...
    for epoch in range(num_epochs):
        ...
        for data in train_dataloader:  # 使用 DataLoader 加载数据8
            ...
            # print("brain_train:", brain.shape)  # torch.Size([64, 64])
            # outputs, brain = model(x1)
            # y = y.view(-1, 1)
            # y = y.float()
        ...
        with torch.no_grad():
            for data in test_dataloader:  # 假设使用 DataLoader 加载测试数据
                ...
                outputs, brain = model(x1, x2, x3, x4, x5)
                # print("brain_test:", brain.shape)  # torch.Size([64, 64])
                # outputs, brain = model(x1)
    ...
    file_name = "subject_{}_brain_model.pth".format(num)
