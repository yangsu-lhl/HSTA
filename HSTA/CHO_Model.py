...
for num in range(1, 53):
    ...
    class MyModel(nn.Module):
        def __init__(self):
            ...
            self.mlp5 = MLP(1230, 615, 1)

            self.mlp = MLP(6720, 3360, 1)
    ...
    # 定义模型参数
    # in_channel = brain_partition[i-1]
    # out_channel = hidden_size = 64
    # input_size = in_channel * 5
    # output_size = feature_dim = 128
    # mlp_input_size = (out_channel * 5 * 3) + (3 * in_channel * 5 * 2)
    # mlp_hidden_size = int(mlp_input_size / 2)
    # mlp_output_size = 1

    dataset_partition = TensorDataset(features_partition1, features_partition2, features_partition3, features_partition4, features_partition5, labels)

    # 把dataset划分训练和测试
    split_ratio = 0.7
    total_samples = len(dataset_partition)
    train_size = int(split_ratio * total_samples)
    test_size = total_samples - train_size
    train_dataset_partition, test_dataset_partition = torch.utils.data.random_split(dataset_partition, [train_size, test_size])

    train_dataloader_partition = DataLoader(train_dataset_partition, batch_size=batch_size, drop_last=True, shuffle=True)
    test_dataloader_partition = DataLoader(test_dataset_partition, batch_size=batch_size, drop_last=True, shuffle=True)
    ...
    for epoch in range(num_epochs):
        total_correct = 0
        total_samples = 0
        for data in train_dataloader_partition:  # 使用 DataLoader 加载数据8
        ...
        # print("下面是被试一的测试集准确率：")
        with torch.no_grad():
            for data in test_dataloader_partition:  # 假设使用 DataLoader 加载测试数据
    ...
    folder_path = "D:\pythonProject\svr/CHO_model"
    file_name = "subject_{}_best_model.pth".format(num)
    file_path = os.path.join(folder_path, file_name)
    torch.save(best_model_state, file_path)

# 创建一个DataFrame对象
df = pd.DataFrame(best_accuracy_list, columns=["CHO_test_best_accuracy"])

# 保存DataFrame为Excel文件
file_path = "CHO_test_best_accuracy.xlsx"  # 文件路径和名称
